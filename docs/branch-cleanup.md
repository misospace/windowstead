...
```bash
./scripts/bin/prune-stale-branches.sh
```
...
- [x] Created `scripts/bin/prune-stale-branches.sh` for future automated cleanup.
...
3. These patterns are recognized by `scripts/bin/prune-stale-branches.sh` for automatic cleanup.
