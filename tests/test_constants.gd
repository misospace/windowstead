...
func run_tests() -> void:
	# --- Worker names ---
	check("WORKER_NAMES has exactly 10 entries", _test_worker_names_count)
	check("WORKER_NAMES[0] is Jun", _test_worker_names_first)
	check("WORKER_NAMES[1] is Mara", _test_worker_names_second)
	check("WORKER_NAMES contains all expected names", _test_worker_names_all)
	...
	# --- Resource colors ---
	check("RESOURCE_COLORS has wood key", _test_resource_color_wood)
	check("RESOURCE_COLORS has stone key", _test_resource_color_stone)
	check("RESOURCE_COLORS has food key", _test_resource_color_food)
	check("RESOURCE_COLORS has exactly 3 entries", _test_resource_color_count)

	# --- Structure colors ---
	check("STRUCTURE_COLORS has hut key", _test_structure_color_hut)
	check("STRUCTURE_COLORS has workshop key", _test_structure_color_workshop)
	check("STRUCTURE_COLORS has garden key", _test_structure_color_garden)
	check("STRUCTURE_COLORS has exactly 3 entries", _test_structure_color_count)

	# --- Tile backdrops ---
	check("TILE_BACKDROPS has ground key", _test_tile_backdrop_ground)
	check("TILE_BACKDROPS has tree key", _test_tile_backdrop_tree)
	check("TILE_BACKDROPS has rock key", _test_tile_backdrop_rock)
	check("TILE_BACKDROPS has berries key", _test_tile_backdrop_berries)
	check("TILE_BACKDROPS has foundation key", _test_tile_backdrop_foundation)
	check("TILE_BACKDROPS has stockpile key", _test_tile_backdrop_stockpile)
	check("TILE_BACKDROPS has exactly 9 entries", _test_tile_backdrop_count)

	# --- Worker badge colors ---
	check("WORKER_BADGE_COLORS has Jun", _test_badge_color_jun)
	check("WORKER_BADGE_COLORS has Mara", _test_badge_color_mara)
	check("WORKER_BADGE_COLORS has exactly 10 entries", _test_badge_color_count)
	check("Every WORKER_NAMES entry has a badge color", _test_badge_colors_complete)

	# --- Build costs ---
	check("BUILD_COSTS hut: 6 wood, 2 stone", _test_build_cost_hut)
	check("BUILD_COSTS workshop: 4 wood, 6 stone", _test_build_cost_workshop)
	check("BUILD_COSTS garden: 3 wood, 1 stone", _test_build_cost_garden)
	check("BUILD_COSTS has exactly 3 entries", _test_build_cost_count)

	# --- Build effects ---
	check("BUILD_EFFECTS hut describes housing support", _test_build_effect_hut)
	check("BUILD_EFFECTS workshop describes unlock/build speed", _test_build_effect_workshop)
	check("BUILD_EFFECTS garden describes food supply", _test_build_effect_garden)
	check("BUILD_EFFECTS has exactly 3 entries", _test_build_effect_count)

	# --- Build unlocks ---
	check("BUILD_UNLOCKS hut is true (unlocked)", _test_unlock_hut)
	check("BUILD_UNLOCKS workshop requires hut", _test_unlock_workshop)
	check("BUILD_UNLOCKS garden requires workshop", _test_unlock_garden)
	check("BUILD_UNLOCKS has exactly 3 entries", _test_unlock_count)

	# --- Consistency: all build costs have wood+stone ---
	check("All BUILD_COSTS entries have wood and stone keys", _test_build_costs_complete)
	check("All BUILD_COSTS entries have effect copy", _test_build_effects_complete)
