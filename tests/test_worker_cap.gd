...
func test_base_cap_no_structures(main: Control) -> void:
	...
	print("--- base cap ---")
	_setup_state(main, [])
	H.assert_eq(main.get_worker_cap(), 2, "base_cap: returns BASE_WORKER_CAP (2) with no builds")
...
func test_one_hut_bonus(main: Control) -> void:
	...
	H.assert_eq(main.get_worker_cap(), 4, "one_hut: base(2) + hut_bonus(2) = 4")
...
func test_multiple_huts_add_up(main: Control) -> void:
	...
	_setup_state(main, builds)
	H.assert_eq(main.get_worker_cap(), 6, "multi_hut: base(2) + 2*hut_bonus(2) = 6")
...
func test_workshop_does_not_increase_cap(main: Control) -> void:
	...
	_setup_state(main, builds)
	H.assert_eq(main.get_worker_cap(), 2, "workshop: no bonus for workshop, stays at base(2)")
...
func test_mixed_structures(main: Control) -> void:
	...
	_setup_state(main, builds)
	H.assert_eq(main.get_worker_cap(), 4, "mixed: base(2) + hut_bonus(2) = 4 (workshop adds nothing)")
...
func test_incomplete_builds_do_not_count(main: Control) -> void:
	...
	_setup_state(main, builds)
	H.assert_eq(main.get_worker_cap(), 2, "incomplete: incomplete hut doesn't count, stays at base(2)")
