...
func run_tests() -> void:
	...
	test_is_goal_complete(RG)
	test_complete_goal_noop_reward(RG)
	test_rotate_after_completion(RG)
	test_reward_preview_text(RG)
...
func test_catalog_exists(gs: Variant) -> void:
	...
	assert_eq(catalog.size(), 7, "catalog_size_7_entries")

	# Check expected types are present
	var types := {}
	for entry in catalog:
		types[entry["type"]] = true
	assert_true(types.has(gs.GOAL_TYPE_RESOURCE), "catalog_has_resource_type")
	assert_true(types.has(gs.GOAL_TYPE_BUILD), "catalog_has_build_type")
	assert_true(types.has(gs.GOAL_TYPE_BUILD_COMPLETE), "catalog_has_build_complete_type")

	# Check all 7 goal IDs
	var ids := ["gather_wood", "gather_stone", "gather_food", "build_hut", "build_workshop", "build_garden", "any_build"]
	for id in ids:
		assert_true(gs.GOAL_CATALOG.any(func(e): return e["id"] == id), "catalog_has_id_%s" % id)
...
func test_apply_goal_template(gs: Variant) -> void:
	...
	assert_eq(goal["id"], "gather_wood", "template_id")
	assert_eq(goal["type"], gs.GOAL_TYPE_RESOURCE, "template_type")
	assert_eq(goal["target"]["resource"], "wood", "template_target_resource")
	assert_eq(goal["target"]["amount"], 10, "template_target_amount")
	assert_eq(goal["current_progress"], 0, "template_progress_starts_zero")
	assert_true(not goal["completed"], "template_not_completed")

	# Verify target is a deep copy (modifying one doesn't affect catalog)
	goal["target"]["amount"] = 999
	assert_eq(gs.GOAL_CATALOG[0]["target"]["amount"], 10, "template_deep_copies_target")
...
func test_select_next_active_goal(gs: Variant) -> void:
	...
	assert_eq(goal["id"], "gather_wood", "select_first_returns_gather_wood")
	assert_true(not goal.is_empty(), "select_returns_non_empty")

	# Skip gather_wood: should return gather_stone
	goal = gs.select_next_active_goal(["gather_wood"])
	assert_eq(goal["id"], "gather_stone", "select_skips_completed")
	...
	assert_eq(goal["id"], "any_build", "select_returns_last_remaining")
...
func test_compute_resource_progress(gs: Variant) -> void:
	...
	assert_eq(goal2["current_progress"], 0, "compute_missing_resource_is_zero")
...
func test_compute_build_complete_progress(gs: Variant) -> void:
	...
	assert_eq(goal["current_progress"], 2, "build_complete_counts_all_builds")
...
func test_is_goal_complete(gs: Variant) -> void:
	...
	goal["current_progress"] = 1
	assert_true(gs.is_goal_complete(goal), "build_complete_complete_with_one_build")
...
func test_complete_goal_noop_reward(gs: Variant) -> void:
	...
	assert_true(not goal["completed"], "goal_not_completed_initially")

	gs.complete_goal(goal)
	assert_true(goal["completed"], "goal_marked_completed")

	# Verify no reward field was added (no-op completion)
	assert_true(not goal.has("reward"), "complete_no_reward_field_added")
...
func test_rotate_after_completion(gs: Variant) -> void:
	...
	assert_eq(new_goal["id"], "gather_stone", "rotate_from_wood_to_stone_no_prior")
	assert_true(active["completed"], "active_goal_marked_completed")
	assert_true(not new_goal["completed"], "new_goal_not_completed")
	assert_eq(new_goal["current_progress"], 0, "new_goal_progress_starts_zero")
	...
	assert_eq(new_goal["id"], "gather_food", "rotate_skips_prior_completed")
	...
	assert_true(new_goal.is_empty(), "rotate_all_done_returns_empty")
	...
	assert_eq(new_goal["id"], "gather_stone", "deduplicate_prior_completed_ids")
	...
	for _i in range(3):
		...
		active = new_goal
	assert_eq(chain.size(), 3, "rotation_chain_length_3")
	assert_eq(chain[0], "gather_stone", "chain_step_1_gather_stone")
	assert_eq(chain[1], "gather_food", "chain_step_2_gather_food")
	assert_eq(chain[2], "build_hut", "chain_step_3_build_hut")
...
func test_reward_preview_text(gs: Variant) -> void:
	...
	assert_eq(preview, "Reward: +1 food", "resource_goal_has_reward_preview")
	...
	assert_eq(preview, "", "goal_without_reward_field_returns_empty")
	...
	assert_eq(preview, "", "goal_with_empty_reward_returns_empty")

	# Test 5: Empty goal dict returns empty
	var empty_goal := {}
	preview = gs.get_reward_preview_text(empty_goal)
	assert_eq(preview, "", "empty_goal_returns_empty")
	...
	preview = gs.get_reward_preview_text(goal)
	assert_true(preview.length() < 40, "reward_preview_fits_compact_dock")
	assert_eq(preview, "Reward: ambient event improves", "garden_goal_reward_preview")
	...
	for entry in gs.GOAL_CATALOG:
		...
		assert_true(not p.is_empty(), "catalog_entry_%s_has_reward" % entry["id"])
