...
## Dock theme/style construction (issue #358) — owns the stylebox helpers and
## the cached button styles.
var dock_theme := DockTheme.new()
...
func create_startup_menu() -> void:
	...
	startup_panel.add_theme_stylebox_override("panel", DockTheme.make_panel_style(Color(0.08, 0.1, 0.14, 0.96), Color(0.44, 0.58, 0.72, 0.95), 16))
...
func _build_stance_panel() -> void:
	...
	stance_panel.add_theme_stylebox_override("panel", DockTheme.make_panel_style(Color(0.11, 0.14, 0.18, 0.92), Color(0.28, 0.34, 0.41, 0.75), 12))
	...
	for stance_key in ColonyStance.ALL_STANCES:
		...
		dock_theme.apply_button_theme(btn)
